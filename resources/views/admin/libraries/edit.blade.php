...
<div class="col-md-8 col-md-offset-2">
    <div class="block block-rounded block-bordered">
        <div class="block-content">
            <form id="publication-update-form" action="{{ route('admin.library.update', $library->id) }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                <div class="form-group required">
                    <label for="question">{{ __('Title') }}</label>
                    <input type="text" name="title" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{ $library->title }}">
                    @if ($errors->has('title'))
                ...
                <div class="form-group">
                  ...
                  <input type="checkbox" name="active" class="form-check-input" {{$library->active == 1 ? 'checked' : ''}}
                  >
              </div>

                <div class="form-group required">
                    <label for="description">{{ __('Description') }}</label>
                    <textarea name="description" id="editor" rows="5" class="form-control">{{ $library->description }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            <img src="{{asset('uploads/libraries/documents.png')}}" height="100">
                            <a class="delete__confirm" href="{{ route('admin.library_image.destroy', $library->file->id) }}"><i class="fa fa-times-circle"></i></a>
                        </div>
                        @else
                        <div class="form-group">
                            <img src="{{asset('uploads/libraries/' . $library->file->filename)}}" height="100">
                            <a class="delete__confirm" href="{{ route('admin.publication_image.destroy', $library->file->id) }}"><i class="fa fa-times-circle"></i></a>
                        </div>
                        @endif
                        @endif
